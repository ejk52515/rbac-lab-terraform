...
$assignments = az role assignment list --scope $vmId --query "[].{role:roleDefinitionName,principal:principalName}" -o json | ConvertFrom-Json
...
# Export report
$verdict = if ($allPass) { "ALL PASS" } else { "FAILURES DETECTED" }
...
Write-Host "Overall: $verdict" -ForegroundColor $color